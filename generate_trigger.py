...
BEGIN
    FOR r IN SELECT routine_schema, routine_name FROM information_schema.routines
             WHERE routine_name LIKE 'notify_pg%'
    LOOP
...
functionTemplate = """
CREATE OR REPLACE FUNCTION {schema}.notify_pg_{table}_{event}() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
  DECLARE
    cur_rec record;
    BEGIN
      PERFORM pg_notify('pg', json_build_object(
                    'table', TG_TABLE_NAME,
    ...
    END;
$$;
DROP TRIGGER IF EXISTS notify_pg_{table}_{event} ON {schema}.{table};
CREATE TRIGGER notify_pg_{table}_{event} AFTER {event} ON {schema}.{table} FOR EACH ROW EXECUTE PROCEDURE {schema}.notify_pg_{table}_{event}();
"""
